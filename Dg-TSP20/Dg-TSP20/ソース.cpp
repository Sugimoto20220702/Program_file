...
#define MAX 9999 // 最大値
#define M_ARGC 1 // 引数
//#define randomize2() srand(time(NULL)) // 乱数初期値設定
//#define randomize() srand(100) // 乱数初期値設定
#define getrandom(MAX, MIN) (rand() % (int)(MAX - MIN + 1) + MIN)
#define getrandom2(MAX, MIN) (rand() % (int)(MAX - MIN + 1) + MIN)
// ↑指定範囲の乱数作成
#define Max 25 // 乱数の最大値
#define Min 5 // 乱数の最小値
#define U_Max 10 // 乱数の最大値
#define U_Min 4 // 乱数の最小値
int Node; // 都市の数
int Capa; //積載量
int* ex; // 行った都市の有無
int** dis; // 距離行列
int* unloading; //各都市の荷下ろし量
int* d_min; // 変更前修繕用
int* r_min; // 通る都市の格納
int d_p; // 総距離
int level; // 再帰の深さ
...
int arrayInit() {
    ...
    unloading = (int*)malloc(sizeof(int) * (Node + 1));
    for (i = 0; i <= Node; i++)
...
int main(int argc, char* argv[]) {
    int num, count = 0;
    fp = fopen("Dg-TSP_30.csv", "w");
    fp2 = fopen("Dg-TSP2_30.csv", "w");
    for (num = 100; num < 110; num++) {
        printf("乱数数 = %d\n", num);
        fprintf(fp, "randam, %d\n", num);
        fprintf(fp2, "random, %d\n", num);
        srand(num);
        int i, n;
        d_p = 0;
        Node = 20;
